...
int cuda_init(CudaContext *ctx) {
    ...
    if (!ctx->hLib) {
        fprintf(stderr, "cuda_drv: cannot load nvcuda.dll\n");
        return -1;
    }
    ...
    printf("device: %s (cc %d.%d), vram: %.0fmb / %.0fmb free\n",
           ctx->devName, ctx->ccMajor, ctx->ccMinor,
           (double)ctx->totalMem / (1024.0 * 1024.0),
           (double)ctx->freeMem / (1024.0 * 1024.0));
